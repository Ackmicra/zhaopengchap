...
import com.uranus.platform.business.jd.entity.pojo.JDRequest2101;
...
public class JdLoanPaymentsServiceImpl implements JdLoanPaymentsService{
	...
	*/  
	@Override
	@WsBaseLog
	public JdResponseView requestfor2102(String projNo, String channelId, String bizContent) {
		//通过项目号查询机构信息
		ProjBaseData projBaseData = projBaseService.getByProjNo(projNo);
		if(projBaseData != null && !StringUtils.isEmpty(projBaseData.getBrNo())) {
			try {
				//将解密后的数据转换成pojo对象
				JDRequest2101 jDRequest2101 = objectMapper.readValue(bizContent,JDRequest2101.class);
		
				//校验View层对象是否为空属性以及长度
				List<String> errorMessList = uranusValidator.validatorAll(jDRequest2101);
				if(errorMessList.size() == 0 ) {
					String brNo = projBaseData.getBrNo();
					//此处返回正确结果，否则一律抛出异常，由异常拦截进行处理
					return requestInfo2102(brNo, jDRequest2101.getApplicationNo());
				}else {
					throw PlatformExceptionFactory.paramInValidateException(CmsBusinessStatus.INVALIDATE_PARAM).build(errorMessList);
				}
			} catch (IOException e) {
	...
	* @param applicationNo  申请号
	*/  
	private JdResponseView requestInfo2102(String brNo, String applicationNo) {
		//根据申请号查询申请信息，并获取批次号
		JdLoanApplyInfoData jdLoanApplyInfoData = jdLoanApplyInfoDataMapper.getByApplicationNo(applicationNo);
		String batchNo = jdLoanApplyInfoData.getBatchNo();
		// 获取流水号
		String tradeNo = createKeyService.getJdTradeNo();
		return lnPaymentsQueryTrans2102Service.request2102(brNo, batchNo, tradeNo, applicationNo);
	}
}
