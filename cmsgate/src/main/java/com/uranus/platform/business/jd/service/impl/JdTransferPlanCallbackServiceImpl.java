...
public class JdTransferPlanCallbackServiceImpl implements JdTransferPlanCallbackService {
	...
	public JdCallbackRequestView transferPlanCallback(String batchNo, int delayLevel, String taskType) {
		...
		JdTransferBatchData jdTransferBatch=null;
		//支付单回调转换京东批次号
		if (CmsBusinessStatus.CALLBACK_PAYMENT_ORDER_RESULT.businessCode().equals(taskType)) {

			batchNumber = jdPaymentOrderDataMapper.getByBatchNo(batchNo).getBatchNumber();

			jdTransferBatch = jdTransferBatchDataMapper.getByBatchNo(batchNumber);

		} else {

			jdTransferBatch = jdTransferBatchDataMapper.getByBatchNo(batchNo);
		...
		if(JdResponseStatus.CHECK_STS.businessCode().equals(checkSts)) {
			...
			if ((JdResponseStatus.TRANSFER_TYPE_ONLINE.businessCode().equals(transferType) 
				...
				jdCallbackRequestView = transferPlanCalllback2313Service.request2313(brNo, batchNo, tradeNo, 
						checkSts, checkDesc, delayLevel, transferType, taskType);
			}else
			...
			if (JdResponseStatus.TRANSFER_TYPE_OUTLINE.businessCode().equals(transferType)
				...
				if (CmsBusinessStatus.CALLBACK_PAYMENT_ORDER_RESULT.businessCode().equals(taskType)) {
					...
					jdCallbackRequestView = transferPlanCalllback2313Service.getTransferPlan(batchNo, checkSts,
							checkDesc);
			...
			if(jdCallbackRequestView != null) {
				...
				if(CmsBusinessStatus.CALLBACK_PAYMENT_ORDER_RESULT.businessCode().equals(taskType)) {
					jdCallbackHttpClient.callbackOrSendMQ(jdCallbackRequestView, batchNo, delayLevel, 
							CmsBusinessStatus.WS_2312.businessCode(), taskType);
				} else if(CmsBusinessStatus.CALLBACK_TRANSFER_PLAN_RESULT.businessCode().equals(taskType)) {
					jdCallbackHttpClient.callbackOrSendMQ(jdCallbackRequestView, batchNo, delayLevel, 
							CmsBusinessStatus.WS_2311.businessCode(), taskType);
				}
			} 
		}
